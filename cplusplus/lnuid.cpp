...
namespace ln_uid {
    ...
    ln_uid_t &createUniqueId(ln_uid_t &out){
        ...
        for(int i=0, j=0; i <time_byte_count; i++, j++) {
            out[i] = timedata[j]; 
        }
        for(int i=5, j=0; i <uid_size; i++, j++) {
            out[i] = randomdata[j]; 
        }
    ...
    void getUniqueIdRange(time_t start, time_t end, ln_uid_t &uid_start, ln_uid_t &uid_end){
        ...
        unsigned long long  int startMilliSecondsSinceEpochTrim =
        (unsigned long long int)(start) * 1000;
        unsigned long long  int endMilliSecondsSinceEpochTrim =
        (unsigned long long int)(end) * 1000;
        ...
        for(int i=0; i <random_byte_count; i++) {
            blankedRandomdata[i] = 0x00;
            filledRandomData[i] = 0xFF;
        }

        for(int i=0, j=0; i <time_byte_count; i++, j++) {
            uid_start[i] = startTimeData[j] ; 
        }
        for(int i=time_byte_count, j=0; i <uid_size; i++, j++) {
            uid_start[i] = blankedRandomdata[j] ; 
        }

        for(int i=0, j=0; i <time_byte_count; i++, j++) {
            uid_end[i] = endTimeData[j] ; 
        }
        for(int i=time_byte_count, j=0; i <uid_size; i++, j++) {
